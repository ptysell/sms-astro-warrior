...
void SN76489_Update(int32_t which, int16_t **buffer, int32_t length)
{
    ...
    for(j = 0; j < length; j++)
    {
        ...
        p->Clock+=p->dClock;
        p->NumClocksForSample=(int32_t)p->Clock;  /* truncates */
        ...
        for (i=0;i<=2;++i) {
            if (p->ToneFreqVals[i]<=0) {   /* If it gets below 0... */
                if (p->Registers[i*2]>PSG_CUTOFF) {
                    ...
                    /* Go to floating point32_t and include the clock fraction for extreme accuracy :D */
                    /* Store as long int, maybe it's faster? I'm not very good at this */
                    p->IntermediatePos[i]=(int32_t)((p->NumClocksForSample-p->Clock+2*p->ToneFreqVals[i])*p->ToneFreqPos[i]/(p->NumClocksForSample+p->Clock)*65536);
